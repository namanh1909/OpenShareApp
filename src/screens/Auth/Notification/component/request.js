...
import { seenAcpPostRequest } from "../../../../redux/reducers/notifyUserSlice";


const DetailRequestNotifyScreen = ({ navigation, route }) => {
    const width = Dimensions.get("window").width;
    const dataUser2 = route?.params?.dataUser2
    ...
    useEffect(() => {
        dispatch(seenAcpPostRequest({ authToken, dataUser: dataUser2 }))
    }, [])

    let itemList = useSelector((state) => state.notify.manegerRequest)
    ...
    return (
        <View style={{ flex: 1, backgroundColor: "#fff" }}>
            ...
            {output && <CustomImageCarousalSquare data={output} pagination={true} />}
            <View
                style={{
                    padding: 10,
                }}
...
export default DetailRequestNotifyScreen;
